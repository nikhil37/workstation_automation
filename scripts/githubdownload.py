...
try:
    ...
    os.system('pip3 install tarfile, gzip, zipfile')
    import tarfile, gzip, zipfile
...
def extract_tar(compressed_data, out_file,name):
    """
        ...
        if gzip_header.startswith(b"\x1f\x8b"):
            with gzip.open(compressed_data, "rb") as gz:
                with tarfile.open(fileobj=gz, mode="r:gz") as tar:
                    tar.extractall(path=f"{out_file}/{name}")    
                    return
        else:
            raise Exception("Unable to identify compression type")
    except Exception as e:
...
def extract_zip(compressed_data, out_file,name):
    """
    ...
    try:
        with zipfile.ZipFile(compressed_data, 'r') as zip_ref:
            ...
            # if len(zip_ref.infolist()) == 1 and zip_ref.infolist()[0].filename == dir:
            #     zip_ref.start_dir = zip_ref.namelist()[0]                            
            zip_ref.extractall(f"{out_file}/{name}")
    except Exception as e:
        raise Exception(e)
...
def get_github_release(repo, regex, output_directory, name=None):
    ...
    try:
        ...
        if url.endswith(".tar.gz"):
            ...
            extract_tar(f_bytes, output_directory,name)
        elif url.endswith(".gz"):
            if not name:
                name = '.'.join(url.split("/")[-1].split('.')[0:-1])
            extract_gz(f_bytes, output_directory, name)
        elif url.endswith(".zip"):
            if not name:
                name = url.split("/")[-1].split('.')[0]
            extract_zip(f_bytes, output_directory,name)
        else:
            if not name:
                name = url.split("/")[-1]
            with open(output_directory + "/" + name, "wb") as f_out:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-x','--regex',required=True)
    parser.add_argument('-o','--output-directory',required=True)
    parser.add_argument('-n','--name',required=False)
    args = parser.parse_args()
    try:            
        get_github_release(args.repo, args.regex, args.output_directory, args.name)
        sys.exit(0)
    except Exception as e:
        print(e)
